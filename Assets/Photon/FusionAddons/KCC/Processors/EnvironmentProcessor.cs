namespace Fusion.Addons.KCC
{
	...
	/// <summary>
	/// Movement implementation for default environment. This imlementation doesn't suffer from position error accumulation due to render delta time integration.
	/// In render update, all properties (speed, velocities, ...) are calculated with fixed delta time. Render delta time is the used for calculation of actual position delta.
	/// Results of this processor in Fixed/Render updates should always be in sync.
	/// </summary>
	public partial class EnvironmentProcessor : KCCProcessor, IPrepareData, ISetDynamicVelocity, ISetKinematicDirection, ISetKinematicTangent, ISetKinematicSpeed, ISetKinematicVelocity, IAfterMoveStep
	{
		// CONSTANTS
